...
from .routers import auth, awx_callbacks, credentials, dashboard, execution_environments, execution_options, host_groups, hosts, inventory_sources, packages, patching, patching_panel, policies, security, services, templates, users
try:
    from .routers import terminal
except ModuleNotFoundError:  # terminal requires Unix-only pty/fcntl/termios
    terminal = None
...
for r in (auth.router, dashboard.router, hosts.router, host_groups.router, users.router, patching.router,
          ...
          services.router, security.router, *(terminal and [terminal.router] or [])):
